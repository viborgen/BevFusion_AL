...
init_train = list(sorted(set(init_train_detect + init_train_track)))
...
def file_collector():
    ...
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/orig_scene.json', 'r') as json_file:
...
def orig_split(random_num = 100):
    ...
    test = list(sorted(set(unlabeled + init_test)))
    ...
    #  # Load existing JSON data from the file
    # with open('bevfusion_bjork/data/nuscenes/v1.0-test/scene.json', 'r') as json_file:
    #     existing_data = json.load(json_file)
    ...
    if selected_data_test:
        ...
        print(f"Object with name '{test}' not found in the data.")
...
def scene_organicer():
    ...
    # Save the combined data to a new JSON file
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/scene.json', 'wb') as json_file:
        #json.dumps(combined_data, json_file, indent=0)
        json_file.write(json.dumps(combined_data, option=json.OPT_SORT_KEYS))
    ...
    # Save the combined data to a new JSON file
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/scene.json', 'wb') as json_file:
...
def sample_organicer():
    ...
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/scene.json', 'r') as f:
    ...
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/orig_sample.json', 'r') as f:
    ...
    # Step 1: Read data from scene.json and extract scene tokens
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/scene.json', 'r') as f:
        scene_data = json.loads(f.read())
        scene_tokens = [scene['token'] for scene in scene_data]

    # Step 2: Read data from samples.json and filter samples by scene tokens
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/orig_sample.json', 'r') as f:
        samples_data = json.loads(f.read())
    ...
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/sample.json', 'wb') as f:
    ...
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/sample.json', 'wb') as f:
...
def sample_data_organicer():
    # Step 1: Read data from scene.json and extract scene tokens
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/sample.json', 'r') as f:
        sample_data = json.loads(f.read())
        sample_tokens = [sample['token'] for sample in sample_data]

    # Step 2: Read data from samples.json and filter samples by scene tokens
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/orig_sample_data.json', 'r') as f:
        sample_data = json.loads(f.read())
        filtered_samples_train = []
        other_samples_train = []
    ...
    # Step 1: Read data from scene.json and extract scene tokens
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/sample.json', 'r') as f:
        sample_data = json.loads(f.read())
        sample_tokens = [sample['token'] for sample in sample_data]

    # Step 2: Read data from samples.json and filter samples by scene tokens
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/orig_sample_data.json', 'r') as f:
        sample_data = json.loads(f.read())
        filtered_samples_test = []
        other_samples_test = []
    ...
    # Step 3: Save the filtered samples into a new JSON file
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/sample_data.json', 'wb') as f:
        # json.dumps(train, f, indent=0)
        f.write(json.dumps(train, option=json.OPT_SORT_KEYS))
        print("Filtered samples have been saved to samples.json")

    # Step 4: Save the other samples into another JSON file
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/sample_data.json', 'wb') as f:
        # json.dumps(test, f, indent=0)
        f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
        print("Other samples have been saved to other_samples.json")
    print("DONE3")
...
def ann_data_organicer():
    ...
    # Step 2: Read data from samples.json and filter samples by scene tokens
    with open('/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/orig_sample_annotation.json', 'r') as f:
        sample_data = json.loads(f.read())
        filtered_samples_train = []
        other_samples_train = []
    ...
    # Step 3: Save the filtered samples into a new JSON file
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/sample_annotation.json', 'wb') as f:
...
def file_gatherer(file):
    global j
    # Step 1: Read data from scene.json and extract scene tokens
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/{file}.json', 'r') as f:
        sensor_data_train = json.loads(f.read())
        

    # Step 2: Read data from samples.json and filter samples by scene tokens
    with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/{file}.json', 'r') as f:
        sensor_data_test = json.loads(f.read())

    test = sensor_data_train + sensor_data_test
    if j == 0: 
    # Step 3: Save the filtered samples into a new JSON file
        with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/calibrated_sensor.json', 'wb') as f:
            #json.dumps(train, f, indent=0)
            f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
        ...
        # Step 4: Save the other samples into another JSON file
        with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/calibrated_sensor.json', 'wb') as f:
            #json.dumps(test, f, indent=0)
            f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
            print(f"{file}.json saved to test folder")
    elif j == 1: 
    # Step 3: Save the filtered samples into a new JSON file
        with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/log.json', 'wb') as f:
            #json.dumps(train, f, indent=0)
            f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
            print(f"{file}.json saved to train folder")
        ...
        # Step 4: Save the other samples into another JSON file
        with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/log.json', 'wb') as f:
            #json.dumps(test, f, indent=0)
            f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
            print(f"{file}.json saved to test folder")
        ...
        # Step 3: Save the filtered samples into a new JSON file
        with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-trainval/ego_pose.json', 'wb') as f:
            #json.dumps(train, f, indent=0)
            f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
            print(f"{file}.json saved to train folder")
        ...
        # Step 4: Save the other samples into another JSON file
        with open(f'/cvrr/bevfusion_bjork/data/nuscenes/v1.0-test/ego_pose.json', 'wb') as f:
            #json.dumps(test, f, indent=0)
            f.write(json.dumps(test, option=json.OPT_SORT_KEYS))
            print(f"{file}.json saved to test folder")
...
for i in range(0, 4):
    ...
    import sys
        ...
        # result = subprocess.run('python create_data.py nuscenes --root-path /cvrr/bevfusion_bjork/data/nuscenes --out-dir /cvrr/bevfusion_bjork/data/nuscenes --extra-tag nuscenes', shell=True, stdout=subprocess.PIPE)

        # # Print the output of the terminal command
    ...
    command3 = 'torchpack dist-run -np 1 python tools/test.py .configs/nuscenes/det/centerhead/lssfpn/camera/256x704/swint/defaultModified.yaml ./pretrained/swint-nuimages-pretrained.pth --eval bbox'
