...
def create_nuscenes_infos(
    ...
    #print(f'Len test: {len(test_scenes)}')
    ...
    unlabeled = "unlabeled" in version
    if test:
        print("test scene: {}".format(len(train_scenes)))
    elif unlabeled:
        print("unlabeled scene: {}".format(len(train_scenes)))
    else:
        print(
    ...
    if test:
        ...
        print("unlabeled sample: {}".format(len(train_nusc_infos)))
        data = dict(infos=train_nusc_infos, metadata=metadata)
        info_path = osp.join(root_path, "{}_infos_unlabeled.pkl".format(info_prefix))
        mmcv.dump(data, info_path)
    else:
        print(
...
def _fill_trainval_infos(nusc, train_scenes, val_scenes, test=False, max_sweeps=10):
    ...
    for sample in mmcv.track_iter_progress(nusc.sample):
        ...
        if sample["scene_token"] in val_scenes: #mathias
            val_nusc_infos.append(info) #mathias
        #else: 
